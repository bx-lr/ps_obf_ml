...
class PS_OBF:
	...
	def _split(self, label_name='is_obf'):
		'''Splits the data into a 80/20 test split'''
		# Get Train and test data
		self.train, self.test = train_test_split(self.data, test_size=.2, random_state=self.rd_seed)
		# Subsplit for featues and lables for train and split 
		self.train_features = self.train[self.train.columns[self.train.columns != label_name]].to_numpy() 
		self.test_features = self.test[self.test.columns[self.test.columns != label_name]].to_numpy()
		self.train_labels = self.train[self.train.columns[self.train.columns == label_name]].to_numpy().ravel()
		self.test_labels = self.test[self.test.columns[self.test.columns == label_name]].to_numpy().ravel()
...
if __name__ == "__main__":
	csv_awl_path = "Z:/Projects/ps_obf_ml/dataset/all_with_labels.csv"
	csv_keyword_sum = "Z:/Projects/ps_obf_ml/dataset/all_with_keyword_sum.csv"
	
	my_data = PS_OBF(csv_keyword_sum, transform=True)
	josh_data = PS_OBF(csv_keyword_sum, transform=False)
	
	my_data_df = my_data.data
	josh_data_df = josh_data.data
	
	my_data_pca_df = my_data.pca_df
	josh_data_pca_df = josh_data.pca_df
'''	
